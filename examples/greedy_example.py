...
means = np.asarray([1,2,10,2,1])
stdevs = np.asarray([3,3,3,3,3])

pessimistic_guess = np.ones((1,5))
optimistic_guess = np.ones((1,5))*5

pes = DiscreteGreedy(initial_values=pessimistic_guess)
opt = DiscreteGreedy(initial_values=optimistic_guess)
eps = DiscreteGreedy(initial_values=pessimistic_guess, eps=0.05)

env = N_ArmedBandit(means=means,stdevs=stdevs)

opt_states, opt_actions, opt_rewards = \
episode(opt.act, env.update_state, env.reward, 0, update_agent=opt.update,
 max_iter=Niter, collect_history=True)

pes_states, pes_actions, pes_rewards = \
episode(pes.act, env.update_state, env.reward, 0, update_agent=pes.update,
 max_iter=Niter, collect_history=True)

eps_states, eps_actions, eps_rewards = \
episode(eps.act, env.update_state, env.reward, 0, update_agent=eps.update,
 max_iter=Niter, collect_history=True)
